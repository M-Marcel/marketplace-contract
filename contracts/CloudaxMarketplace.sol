...
error TransferNotCompleted(uint256 senderBalance, uint256 fundTosend);
...
contract CloudaxMarketplace is ReentrancyGuard, ERC721URIStorage, Ownable {
        ...
        isReady(collectionId, _qty)
    {
        ...
        uint256 royalty = msg.value.mul(_qty).mul(collection.creatorFee).div(
            100
        );
        ...
        // Store the mapping of the ID a sold item's copy or token id to the Item being purchased. {update and a loop logic}
